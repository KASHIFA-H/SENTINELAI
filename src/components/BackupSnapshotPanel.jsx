...
import { HardDrive, RotateCcw, CheckCircle, Plus, Clock, ChevronDown, ChevronRight, FileText, Lock, Shield, RefreshCw } from 'lucide-react'
...
function FileRow({ f }) {
  ...
  return (
    <div className="flex items-center gap-2 px-3 py-1.5 rounded transition-colors"
      ...
      <span className="text-xs font-mono flex-1 truncate" style={{ color: f.status === 'clean' ? '#3d3020' : color }}>{f.name}</span>
      <span className="text-xs shrink-0" style={{ color: '#6b5a45' }}>{(f.size / 1024).toFixed(1)} KB</span>
      <span className="text-xs px-1.5 py-0.5 rounded shrink-0"
        style={{ color, background: `${color}15`, border: `1px solid ${color}30` }}>{f.status}</span>
    </div>
  )
}
  ...
  return (
    <div className="rounded-xl overflow-hidden mb-2" style={{ border: '1px solid #D1BFA2' }}>
      <div className="flex items-center gap-3 px-4 py-3 cursor-pointer transition-colors"
        ...
        onClick={toggle}>
        <button className="shrink-0 transition-colors" style={{ color: '#6b5a45' }}>
          {expanded ? <ChevronDown size={14} /> : <ChevronRight size={14} />}
        </button>
        <span className="w-2.5 h-2.5 rounded-full shrink-0"
          style={{ background: isRestored ? '#2d6a4f' : '#C2A68D' }} />
        ...
        {isRestored ? (
          ...
          <button onClick={e => { e.stopPropagation(); onRestore(snap.snapshot_id) }}
          ...
          {loadingFiles ? (
            ...
            <div className="py-2">
              <div className="flex items-center gap-4 px-4 py-2 mb-1 border-b" style={{ borderColor: '#D1BFA2' }}>
                ...
                {lockedFiles.length > 0 && <span className="text-xs" style={{ color: '#c0392b' }}>✗ {lockedFiles.length} encrypted</span>}
                <span className="text-xs ml-auto" style={{ color: '#6b5a45' }}>{files.length} total</span>
              </div>
              <div className="px-2">{files.map((f, i) => <FileRow key={i} f={f} />)}</div>
            </div>
          )}
        </div>
      )}
...
export default function BackupSnapshotPanel() {
        ...
        api.snapshots().then(d => {
          ...
          if (local.length) {
            setSnaps(local.map(id => ({
              ...
              file_count: 0, restore_status: 'available',
            })))
          }
          setLoading(false)
        })
  ...
  return (
    <div className="rounded-xl p-5 flex flex-col w-full" style={{ background: '#FFFFFF', border: '1px solid #D1BFA2', height: 380 }}>
      <div className="flex items-center gap-2 mb-4">
        ...
        <div className="ml-auto flex items-center gap-2">
          ...
          <button onClick={() => api.takeSnapshot().then(load)}
      ...
      <div className="flex-1 overflow-y-auto scrollbar-thin" style={{ minHeight: 0 }}>
        {snaps.length === 0
          ? <p className="text-xs text-center py-6" style={{ color: '#6b5a45' }}>No snapshots yet</p>
          : snaps.map(s => <SnapshotRow key={s.snapshot_id} snap={s} onRestore={restore} restoring={restoring} />)
        }
      </div>
    </div>
  )
