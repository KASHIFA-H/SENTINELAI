...
const INITIAL_HISTORY = [
  { url: 'http://microsoft-account-verify.top/login',    verdict: 'MALICIOUS',  risk_score: 95, time: '09:14:32' },
  { url: 'https://paypa1.com/secure/login',              verdict: 'MALICIOUS',  risk_score: 85, time: '09:22:10' },
  { url: 'https://google-security-alert.xyz/verify',     verdict: 'MALICIOUS',  risk_score: 90, time: '10:31:17' },
  { url: 'https://bit.ly/3xR9mKp',                      verdict: 'SUSPICIOUS', risk_score: 45, time: '11:02:58' },
  { url: 'https://microsoft.com',                        verdict: 'SAFE',       risk_score: 0,  time: '12:00:00' },
]
  ...
  return (
    <div className="rounded-xl p-5" style={{ background: '#FFFFFF', border: '1px solid #D1BFA2' }}>
      <div className="flex items-center gap-2 mb-4">
        ...
        <p className="text-xs font-semibold tracking-widest uppercase" style={{ color: '#6b5a45' }}>URL Threat Scanner</p>
      ...
      <div className="flex flex-wrap gap-1.5 mb-3">
        {DEMO_URLS.map(d => (
          <button key={d.url} onClick={() => { setUrl(d.url); scan(d.url) }}
            className="text-xs px-2 py-1 rounded-lg transition-all"
            style={{ background: '#F5F5DC', border: '1px solid #D1BFA2', color: '#6b5a45' }}
            onMouseEnter={e => { e.currentTarget.style.borderColor = '#C2A68D'; e.currentTarget.style.color = '#1a1a1a' }}
            onMouseLeave={e => { e.currentTarget.style.borderColor = '#D1BFA2'; e.currentTarget.style.color = '#6b5a45' }}>
            {d.label}
          </button>
        ))}
      </div>
      ...
      <div className="flex gap-2 mb-4">
        <div className="relative flex-1">
          ...
          <input value={url} onChange={e => { setUrl(e.target.value); setResult(null) }}
      ...
      {result && style && (
        <div className="rounded-xl p-4 mb-4 animate-slide-in"
            ...
            <div className="space-y-1.5 mb-3">
              ...
              {result.reasons.map((r, i) => (
                <div key={i} className="flex items-start gap-2 px-3 py-2 rounded-lg"
                  style={{ background: 'rgba(255,255,255,0.6)', borderLeft: `3px solid ${style.color}` }}>
      ...
      {history.length > 0 && (
        <div>
          <p className="text-xs font-semibold uppercase tracking-wider mb-2" style={{ color: '#6b5a45' }}>Recent Scans</p>
